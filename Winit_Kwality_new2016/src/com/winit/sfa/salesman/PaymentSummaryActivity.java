...
public class PaymentSummaryActivity extends BaseActivity
{
	...
	private LinkedHashMap<String,Customer_InvoiceDO> hmCashDetails = new LinkedHashMap<String,Customer_InvoiceDO>();
	private LinkedHashMap<String,Customer_InvoiceDO> hmChequeDetails = new LinkedHashMap<String,Customer_InvoiceDO>();
...
//    	tvFromDate.setTag(selectedDate);
		...
		btnPrint_Summarry.setOnClickListener(new OnClickListener()
		{
			...
			public void onClick(View v) 
			{
				if((hmCashDetails == null || hmCashDetails.isEmpty()) && (hmChequeDetails == null || hmChequeDetails.isEmpty()))
					showCustomDialog(PaymentSummaryActivity.this, "Alert!", "There is no summary to print.", "OK", "", "");
				else
				{
	...
	private void loadPayments()
	{
		...
		new Thread(new Runnable() 
		{
			...
			public void run() 
			{
				final Object[] objects = new PaymentSummeryDA().getPaymentSummary(fromDate,toDate);
				hmCashDetails = (LinkedHashMap<String,Customer_InvoiceDO>) objects[0];
				hmChequeDetails = (LinkedHashMap<String,Customer_InvoiceDO>) objects[1];
...
//			showCustomDialog(PaymentSummaryActivity.this, getResources().getString(R.string.warning), "There is no payment summary to print.", getResources().getString(R.string.OK), null, "");
	...
	protected void ShowOptionPopup()
	{
		...
		final CustomDialog mCustomDialog = new CustomDialog(PaymentSummaryActivity.this, view, preference .getIntFromPreference("DEVICE_DISPLAY_WIDTH", 320) - 40,
	...
	{
		private LinkedHashMap<String,Customer_InvoiceDO> hmDetails;
		...
		public Fragment getItem(int position)
		{
			...
			PaymentSummaryFragment paymentSummaryFragment = new PaymentSummaryFragment(hmDetails, PaymentSummaryActivity.this);
		    ...
		    public void onDateSet(DatePicker view, int yearSel, int monthOfYear, int dayOfMonth) 
		    {
		    	...
		    	if(!selectedDate.equalsIgnoreCase(fromDate))
		    	{
		    		...
		    		if(CalendarUtils.getDiffBtwDatesInDays(selectedDate,toDate) >= 0)
		    		{
		    			...
		    			showCustomDialog(PaymentSummaryActivity.this,"Alert",getString(R.string.from_date_should_not_be_greater_than_to_date),getString(R.string.OK),null,null);
		    ...
		    public void onDateSet(DatePicker view, int yearSel, int monthOfYear, int dayOfMonth) 
		    {
		    	...
		    	if(!selectedDate.equalsIgnoreCase(toDate))
		    	{
		    		if(CalendarUtils.getDiffBtwDatesInDays(fromDate,selectedDate) >= 0)
		    		{
		    			...
		    			showCustomDialog(PaymentSummaryActivity.this,"Alert",getString(R.string.to_date_should_not_be_lesser_than_from_date),getString(R.string.OK),null,null);
		    	}
		    }
	    };
}
