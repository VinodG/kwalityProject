...
public class VehicleList extends BaseActivity
{
	...
	private LinearLayout llTruck_List, llLayoutMiddle, llSubheader,llMonthForfarmance,llPagerTab;
	private TextView tvHead,tvMonthPerformance,tvFrequncyHeader,tvRoutePerformance, /*tvFrequncy, */tvDeliveryDateValue, tvDateOfJourney, tvNoOrderFound/*,tvFirstSunDate,tvSecondSunDate,tvThirdSunDate,tvFourthSunDate,tvTargetAchived,tvFirstSunTarget,tvSecondSunTarget,tvThirdSunTarget,tvFourthSunTarget*/;
	private ListView lvTruckList;
	private TruckListAdapter orListAdapter;
	private Vector<VehicleDO> vecTruckList;
	private int scheduledCalls=0, productiveCalls=0, actualCallsScheduled=0,actualCallsUnScheduled=0, zeroSalesOutlet=0, actualCalls=0;
	private int productiveCallsPlanned=0, productiveCallsUnPlanned = 0,zeroSalesOutletPlanned=0,zeroSalesOutletunPlanned=0;
	...
	private LinearLayout llMfdToday,llTotalCollection;
	private ImageView ivMtd,ivToday;
	
	private TextView tvSellingEfficiency, tvCovragePercentage,tvActualCallsPlanned ,
					tvActualCalls, tvSchudeduledCalls,tvProductiveCallsPlanned,tvProductiveCallsUnplanned,tvZeroSalesPlanned,
					tvZeroSalesUnplanned,tvSchedulecallTitle,tvActualCallsTitle,tvActualCallsPlannedTitle,
					tvActualCallsUnplannedTitle, tvTotalActualCallsUnplanned, tvTotalCollection, tvTotalSales;
	...
	private void setUserPerformanceAdapter()
	{
		...
		pager.setOnPageChangeListener(new OnPageChangeListener()
		{
			...
			public void onPageSelected(int postion) 
			{
				...
				{
					llMfdToday.setVisibility(View.GONE);
					tvMonthPerformance.setText("Agency wise Performance");
				}
				else if(postion==4)
				{
					llMfdToday.setVisibility(View.GONE);
	...
	private LinearLayout getProductCatlogView(int position)
	{
		...
		case 0:
			...
			try
			{
				...
				/*tvFoodAchived.setText(decimalFormat.format(targetAcheivement[6]));
...
//				String pieChartHTMLCode = getPieChartDataValues();
	...
	{
		int Totalcount=0,visitedcount=0;
		Calendar c = Calendar.getInstance();
...
//		int date = c.get(Calendar.DAY_OF_MONTH);
	...
	//Regions	
		...
		{
			coverageReport.visitcount      =  (visitedcount*100)/1;
			
			coverageReport.unvisitcount    = ((Totalcount-visitedcount)*100)/1;
		}
...
//		unperfectcount=12;
	...
	private void refreshPageController() 
	{
		...
		for (int i = 0; i <= (adapter.getCount()-1); i++)
		{
			final ImageView imgvPagerController = new ImageView(VehicleList.this);
	...
	private String getDataPerfectStoreOLd(PerfectStore perfectStore) {
		...
		String test = "<?xml version=\"1.0\" encoding=\"iso-8859-1\"?><!DOCTYPE html PUBLIC \"-//W3C//DTD XHTML 1.0 Transitional//EN\" \"http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd\">\n"
				...
				+ "<br/>"
				+ "<br/>"
				+ "<div style=\"margin: 0px 0px 0px 160px;\">\n"
				+ "<div class=\"MainDiv\" style=\"width: 150px; height: 150px; float: left;\">\n<div class=\"Box\" style=\"width: 25px; height: 25px; background:#009900; float: left; margin:-30px 5px 0px 0px;\">\n</div>\n<label style=\"width: auto; float: left; font-size: 13px; font-family: arial;margin-top:-25px;margin-left:30px;\">\nPerfect Stores - "
				+ perfectStore.perfectstorecount
				+ "</label>\n</div>"
				+ " </div>\n"
				+ "<div style=\"margin: 0px 0px 0px 0px;\">\n"
	...
	private XYMultipleSeriesDataset getTruitonBarDataset() 
	{
		...
		//String barsTitle[] = {"Target","Actual"};
		double[] targetValues = {9.5,7.5,7.5,6,6.5,5.5,5,4.5};
		double[] actualValues = {8,7,6.5,6,6,5,4.5,4};
		
		CategorySeries seriesTarget = new CategorySeries("");
		CategorySeries  seriesActual = new CategorySeries("");
		for (int k = 0; k < nr; k++) 
		{
			seriesTarget.add(targetValues[k]);
				
		 }
		
		for (int k = 0; k < nr; k++) 
		{
			seriesActual.add(actualValues[k]);
...
//		{  
//			CategorySeries series = new CategorySeries(barsTitle[i]);
//			for (int k = 0; k < nr; k++) 
//			{
//				if(i == 0)
//			      series.add(targetValues[k]);
//				else if(i == 1)
//					series.add(actualValues[k]);
//		    }
		    dataset.addSeries(seriesTarget.toXYSeries());
		    dataset.addSeries(seriesActual.toXYSeries());
//		}
	    return dataset;
	}
	...
	public XYMultipleSeriesRenderer getTruitonBarRenderer() 
	{
		...
		r.setColor(Color.parseColor("#FFDA17"));
		renderer.addSeriesRenderer(r);
		r = new SimpleSeriesRenderer();
		r.setColor(Color.parseColor("#219A3E"));
		renderer.addSeriesRenderer(r);
		return renderer;
	}
	...
	private void myChartSettings(XYMultipleSeriesRenderer renderer) 
	{
		...
		renderer.addXTextLabel(1, "Agency1");
		renderer.addXTextLabel(2, "Agency2");
		renderer.addXTextLabel(3, "Agency3");
		renderer.addXTextLabel(4, "Agency4");
		renderer.addXTextLabel(5, "Agency5");
		renderer.addXTextLabel(6, "Agency6");
		renderer.addXTextLabel(7, "Agency7");
		renderer.addXTextLabel(8, "Agency8");
