...
public class CustomerOrderSummary extends BaseActivity 
{
...
//	private LinearLayout llDateFilterFrom,llDateFilterTo;
	...
	public void initialize() 
	{
		...
		else
		{
			...
			etSearch.setHint("Search by Invoice Number");
		}
...
//									trxType = TrxHeaderDO.get_TRXTYPE_SAVED_ORDER();
	...
	public void intializeControls()
	{
		...
		preference				=   new Preference(CustomerOrderSummary.this);
...
//		llDateFilterFrom	=	(LinearLayout) llDeliveryStatus.findViewById(R.id.llDateFilterFrom);
		...
		pager.setOnPageChangeListener(CustomerOrderSummary.this);
		...
		new Thread(new Runnable() 
		{
			...
			public void run() 
			{
				synchronized (SUMMARY_LOCK) 
				{
					...
					hmOrders = hmOrdersData  = new CommonDA().getCustomerOrderSummary(empNO,fromDate,toDate,mallsDetailss.site);
					final Object obj[] =  new CommonDA().getSumupOfOrdersForOneClient(empNO,fromDate,toDate,mallsDetailss.site,preference.getStringFromPreference(Preference.SALESMAN_TYPE, "").equalsIgnoreCase(preference.PRESELLER));
					runOnUiThread(new Runnable() 
					{
						@Override
						public void run() 
...
//					trxType = TrxHeaderDO.get_TRXTYPE_SAVED_ORDER();
			...
			/**
			 * private String[] tabsName = {"SALES","SAVED","RETURN","MISSED","ADVANCE","FOC"};
			 * private String[] tabsNamePreseller = {"SALES","ADVANCE","FOC"};
			 */
			return new ListOrderFragment(CustomerOrderSummary.this,position,hmOrders.get(trxType),tabs,false, mallsDetailss);
		}
	}
	    ...
	    public void onDateSet(DatePicker view, int yearSel, int monthOfYear, int dayOfMonth) 
	    {
	    	...
	    	if(!selectedDate.equalsIgnoreCase(fromDate))
	    	{
	    		...
	    		if(CalendarUtils.getDiffBtwDatesInDays(selectedDate,toDate) >= 0)
	    		{
	    			...
	    			showCustomDialog(CustomerOrderSummary.this,"Alert",getString(R.string.from_date_should_not_be_greater_than_to_date),getString(R.string.OK),null,null);
	    ...
	    public void onDateSet(DatePicker view, int yearSel, int monthOfYear, int dayOfMonth) 
	    {
	    	...
	    	if(!selectedDate.equalsIgnoreCase(toDate))
	    	{
	    		if(CalendarUtils.getDiffBtwDatesInDays(fromDate,selectedDate) >= 0)
	    		{
	    			...
	    			showCustomDialog(CustomerOrderSummary.this,"Alert",getString(R.string.to_date_should_not_be_lesser_than_from_date),getString(R.string.OK),null,null);
	    	}
			...
			public void run() 
			{
				...
				runOnUiThread(new Runnable() 
				{
					...
					public void run() 
					{
						...
						{
							tabs.setTabText(1, vecOrderList.size());
							tabs.getTag(1);//1 for saved order position
						}
