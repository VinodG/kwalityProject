...
public class VanStockNewLoadRequest extends BaseActivity
{
	...
	public void initialize() 
	{
		...
		btnOrdersheetVerify.setOnClickListener(new OnClickListener() 
		{
			...
			public void onClick(View v) 
			{
				...
				{
					Intent intent = new Intent(VanStockNewLoadRequest.this, VerifyItemInVehicle.class);
//					intent.putExtra("updatededDate", vecOrdProduct);
				...
				else if(load_type == AppConstants.UNLOAD_STOCK)
				{
					if(validateItems(vecOrdProduct))
					{
						...
						Intent intent = new Intent(VanStockNewLoadRequest.this, PreviewNewLoadRequest.class);
...
//						insertUpdateRequest(vecOrdProduct);
						...
						showCustomDialog(VanStockNewLoadRequest.this, getString(R.string.warning), "Please select atleast one item for unload request.", "OK", null, "");
				}
				else if(vecOrdProduct != null && vecOrdProduct.size() > 0)
				{
					/**
					 * Check whether the Salesman is selected.
					 * From whom the request for Van Item Transfer comes.
					 */
					if(!(objWareHouseDetail != null && objWareHouseDetail.WareHouseCode != null && !objWareHouseDetail.WareHouseCode.equalsIgnoreCase("")))
					{
						showCustomDialog(VanStockNewLoadRequest.this, getString(R.string.warning), "Please select atleast one salesman", "OK", null, "");
					}
					else if(validateItems(vecOrdProduct))
					{
						PreviewVanloadDO objPreviewVanloadDO = getSelectedItems(vecOrdProduct);
						Intent intent = new Intent(VanStockNewLoadRequest.this, PreviewNewLoadRequest.class);
						intent.putExtra("load_type", load_type);
...
//						insertUpdateRequest(vecOrdProduct);
						...
						showCustomDialog(VanStockNewLoadRequest.this, getString(R.string.warning), "Please select atleast one item having quantity greater than zero.", "OK", null, "");
				}
				else
				{
					if(load_type == AppConstants.LOAD_STOCK)
						showCustomDialog(VanStockNewLoadRequest.this, getString(R.string.warning), "Please select atleast one item for load request.", "OK", null, "");
					else
						showCustomDialog(VanStockNewLoadRequest.this, getString(R.string.warning), "Please select atleast one item for unload request.", "OK", null, "");
				}
			}
		...
		btnOrdersheetAddNew.setOnClickListener(new OnClickListener()
		{
			...
			public void onClick(View v) 
			{
				...
				{
					showCustomDialog(VanStockNewLoadRequest.this, getResources().getString(R.string.warning), "Please select a Warehouse.", getResources().getString(R.string.OK), null, "search");
				}
				else
				{
//					showAddNewSkuPopUp();
...
//					vecWareHouseDetail = new WareHouseDA().getAllWareHouseforthisSalesPerson(preference.getStringFromPreference(preference.ORG_CODE, ""));
					vecWareHouseDetail = new TransferUserDA().getAllUser(preference.getStringFromPreference(Preference.USER_ID, ""));
				}
...
//										showWareHouseSelector();
	...
	{
		CustomBuilder builder = new CustomBuilder(VanStockNewLoadRequest.this, "Select Warehouse", true);
...
//    				showAddNewSkuPopUp();
			...
			public void run()
			{
				...
				if(!TextUtils.isEmpty(movementId))
				{
					...
					runOnUiThread(new Runnable()
					{
						...
						public void run() 
						{
							hideLoader();
							if(load_type == AppConstants.LOAD_VAN_TRANSFER_STOCK)
								showCustomDialog(VanStockNewLoadRequest.this, "Successful !", "Load request has been made successfully.", "OK", null, "finish", false);

							else 
								showCustomDialog(VanStockNewLoadRequest.this, "Successful !", "Unload request has been made successfully.", "OK", null, "finish", false);
								uploadData();
						}
					});
				}
				else
				{
					runOnUiThread(new Runnable()
					{
						...
						{
							hideLoader();
							showCustomDialog(VanStockNewLoadRequest.this, "Alert !", "Please try after some time.", "OK", null, "");
						}
					});
				}
			}
	...
	public void intialiseControls()
	{
        ...
        tvUOM.setText("UOM");
        tvWareHouse.setText("Select Salesman");
...
//    		ivSepCheck.setVisibility(View.VISIBLE);
        ...
        else
        {
    		...
    		ivSepUN.setVisibility(View.GONE);
...
//							inventoryItemAdapter.refresh(vecOrdProduct, llOrderListView);
	...
	protected void onResume() {
		...
		if(loadRequestDO == null)
		{
			tvOrdersheetHeader.setText("Add New Van Transfer Request");
	...
	public void onBackPressed() {
		if (loadRequestDO==null && load_type == AppConstants.LOAD_STOCK && ordersheetadapter != null
			...
			showCustomDialog(VanStockNewLoadRequest.this, getString(R.string.warning), "Are you sure you don't want to save current data?", getString(R.string.Yes), getString(R.string.No), "GoBack");
		} else{
			super.onBackPressed();
		}
	}
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) 
	{
		...
		{
			Intent intent = new Intent(VanStockNewLoadRequest.this, ScanItemActivity.class);
	...
	{
		if(isNetworkConnectionAvailable(VanStockNewLoadRequest.this))
		{
...
//					loadVanStock_Sync("Loading Stock...", empNo);
					...
					runOnUiThread(new Runnable()
					{
						...
						public void run() 
						{
							...
							case LoadRequestDO.MOVEMENT_STATUS_PENDING:
								tvOrdersheetHeader.setText("Movement Code: - "+loadRequestDO.MovementCode+" (Pending)");
								showCustomDialog(VanStockNewLoadRequest.this, "Warning !", "Request is still pending, Please try after some time.", "OK", null, "");
								break;
							case LoadRequestDO.MOVEMENT_STATUS_APPROVED:
								showCustomDialog(VanStockNewLoadRequest.this, "Successful", "Request has been approved.", "OK", null, "Verify");
								tvOrdersheetHeader.setText("Movement Code: - "+loadRequestDO.MovementCode+" (Approved)");
								break;
							case LoadRequestDO.MOVEMENT_STATUS_APPROVED_VERIFY:
								tvOrdersheetHeader.setText("Movement Code: - "+loadRequestDO.MovementCode+" (Collected)");
							...
							case LoadRequestDO.MOVEMENT_STATUS_REJECTED:{
								tvOrdersheetHeader.setText("Movement Code: - "+loadRequestDO.MovementCode+" (Rejected)");
								if(load_type == AppConstants.LOAD_STOCK)
									showCustomDialog(VanStockNewLoadRequest.this, "Alert!", "Your Load request has been rejected by admin.", "OK", null, "finish");
								else
									showCustomDialog(VanStockNewLoadRequest.this, "Alert!", "Your Unload request has been rejected by admin.", "OK", null, "finish");
			...
			showCustomDialog(VanStockNewLoadRequest.this, "Warning !", getString(R.string.no_internet), "OK", null, "");
	}
...
//				viewHolder.ivAcceptCheckItems11.setVisibility(View.VISIBLE);
//				viewHolder.sep11.setVisibility(View.VISIBLE);
			}
			else
			{
...
//				viewHolder.ivAcceptCheckItems11.setVisibility(View.GONE);
//				viewHolder.sep11.setVisibility(View.GONE);
			}
			
			
//			if(productDO.itemChecked)
//				viewHolder.ivAcceptCheckItems11.setImageResource(R.drawable.checkbox);
//			else
//				viewHolder.ivAcceptCheckItems11.setImageResource(R.drawable.uncheckbox);
...
//				int recomended = 0;
//				if(productDO.RecomendedLoadQuantity != null)
//					recomended = StringUtils.getInt(productDO.RecomendedLoadQuantity);
//				
//				LogUtils.debug("recomended", ""+recomended);
//				viewHolder.tvVanRecomended.setText(""+recomended/(int)productDO.eaConversion);
			}
	...
	private static class ViewHolder
	{
	 ...
	 {
		 CustomBuilder customBuilder = new CustomBuilder(VanStockNewLoadRequest.this, title, false);
