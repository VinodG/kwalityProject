...
public class PreviewNewLoadRequest extends BaseActivity
{
...
//	private Vector<VanLoadDO> vecSearchedItemd;
	...
	PreviewVanloadDO objPreviewVanloadDO;
	String POST_LOAD = "POST_LOAD";
	...
	public void initialize() 
	{
		...
		if(bundle != null)
		{
			...
			if(bundle.containsKey("arrvanload"))
				objPreviewVanloadDO = (PreviewVanloadDO) getIntent().getExtras().get("arrvanload");
			if(bundle.containsKey("objWareHouseDetail"))
				objWareHouseDetail = (WareHouseDetailDO) getIntent().getExtras().get("objWareHouseDetail");
			
			if(getIntent().hasExtra(AppConstants.DIVISION))
				Division = getIntent().getExtras().getInt(AppConstants.DIVISION);
		}
		
		if(objPreviewVanloadDO != null)
			vecOrdProduct = objPreviewVanloadDO.arrvanload;
		...
		if(load_type== AppConstants.LOAD_STOCK)
			tvOrdersheetHeader.setText("Load Request Preview");
		else if(load_type== AppConstants.UNLOAD_STOCK)
			tvOrdersheetHeader.setText("Unload Request Preview");
//		tvOrdersheetHeader.setText(tvOrdersheetHeader.getText().toString());
		...
		btnOrdersheetVerify.setOnClickListener(new OnClickListener() 
		{
			...
			{
				synchronized (POST_LOAD)
				{
					btnOrdersheetVerify.setClickable(false);
					btnOrdersheetVerify.setEnabled(false);
					
					llOrdersheetVerify.setClickable(false);
					llOrdersheetVerify.setEnabled(false);
					...
					{
						Intent intent = new Intent(PreviewNewLoadRequest.this, VerifyItemInVehicle.class);
//						intent.putExtra("updatededDate", vecOrdProduct);
					...
					else if(load_type == AppConstants.UNLOAD_STOCK)
					{
						if(validateItems(vecOrdProduct))
							...
							showCustomDialog(PreviewNewLoadRequest.this, getString(R.string.warning), "Please select atleast one item for unload request.", "OK", null, "");
					}
					else if(vecOrdProduct != null && vecOrdProduct.size() > 0)
					{
						if(validateItems(vecOrdProduct))
							...
							showCustomDialog(PreviewNewLoadRequest.this, getString(R.string.warning), "Please select atleast one item having quantity greater than zero.", "OK", null, "");
					}
					else
					{
						if(load_type == AppConstants.LOAD_STOCK)
							showCustomDialog(PreviewNewLoadRequest.this, getString(R.string.warning), "Please select atleast one item for load request.", "OK", null, "");
						else
							showCustomDialog(PreviewNewLoadRequest.this, getString(R.string.warning), "Please select atleast one item for unload request.", "OK", null, "");
					}
				}
...
//			vecOrdProduct = new ArrayList<VanLoadDO>();
		...
		tvWareHouse.setVisibility(View.GONE);
		...
		btnOrdersheetAddNew.setOnClickListener(new OnClickListener()
		{
			...
			public void onClick(View v) 
			{
				...
				{
					showCustomDialog(PreviewNewLoadRequest.this, getResources().getString(R.string.warning), "Please select a Warehouse.", getResources().getString(R.string.OK), null, "search");
				}
				else
				{
//					showAddNewSkuPopUp();
...
//										showWareHouseSelector();
	...
	{
		CustomBuilder builder = new CustomBuilder(PreviewNewLoadRequest.this, "Select Warehouse", true);
		...
		new Thread(new Runnable()
		{
			...
			public void run()
			{
				...
				if(!TextUtils.isEmpty(movementId))
				{
					...
					loadRequestDO.UserLoadType	=	""+load_type;
					...
					loadRequestDO.Division		=	Division;
					...
					runOnUiThread(new Runnable()
					{
						...
						public void run() 
						{
							hideLoader();
							if(load_type == AppConstants.LOAD_STOCK)
								showCustomDialog(PreviewNewLoadRequest.this, "Successful !", "Load request has been made successfully.", "OK", null, "finish", false);

							else if(load_type == AppConstants.LOAD_VAN_TRANSFER_STOCK)
								showCustomDialog(PreviewNewLoadRequest.this, "Successful !", "Transfer Load request has been made successfully.", "OK", null, "finish", false);

							else
								showCustomDialog(PreviewNewLoadRequest.this, "Successful !", "Unload request has been made successfully.", "OK", null, "finish", false);
				...
				else
				{
					runOnUiThread(new Runnable()
					{
						...
						{
							hideLoader();
							showCustomDialog(PreviewNewLoadRequest.this, "Alert !", "Please try after some time.", "OK", null, "");
						}
					});
				}
			}
...
//							inventoryItemAdapter.refresh(vecOrdProduct, llOrderListView);
	...
	protected void onResume() {
		...
		if(loadRequestDO == null)
		{
//			tvOrdersheetHeader.setText("Add New Request Preview");
			if(load_type== AppConstants.LOAD_STOCK)
				tvOrdersheetHeader.setText("Load Request Preview");
			else if(load_type== AppConstants.UNLOAD_STOCK)
				tvOrdersheetHeader.setText("Unload Request Preview");
	...
	public void onBackPressed() {
		/*if (loadRequestDO==null && load_type == AppConstants.LOAD_STOCK && ordersheetadapter != null
			...
			showCustomDialog(PreviewNewLoadRequest.this, getString(R.string.warning), "Are you sure you don't want to save current data?", getString(R.string.Yes), getString(R.string.No), "GoBack");
		} else*/{
			super.onBackPressed();
		}
	}
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) 
	{
		...
		{
			Intent intent = new Intent(PreviewNewLoadRequest.this, ScanItemActivity.class);
	...
	{
		if(isNetworkConnectionAvailable(PreviewNewLoadRequest.this))
		{
...
//					loadVanStock_Sync("Loading Stock...", empNo);
					...
					runOnUiThread(new Runnable()
					{
						...
						public void run() 
						{
							...
							case LoadRequestDO.MOVEMENT_STATUS_PENDING:
								tvOrdersheetHeader.setText("Movement Code: - "+loadRequestDO.MovementCode+" (Pending)");
								showCustomDialog(PreviewNewLoadRequest.this, "Warning !", "Request is still pending, Please try after some time.", "OK", null, "");
								break;
							case LoadRequestDO.MOVEMENT_STATUS_APPROVED:
								showCustomDialog(PreviewNewLoadRequest.this, "Successful", "Request has been approved.", "OK", null, "Verify");
								tvOrdersheetHeader.setText("Movement Code: - "+loadRequestDO.MovementCode+" (Approved)");
								break;
							case LoadRequestDO.MOVEMENT_STATUS_APPROVED_VERIFY:
								tvOrdersheetHeader.setText("Movement Code: - "+loadRequestDO.MovementCode+" (Collected)");
							...
							case LoadRequestDO.MOVEMENT_STATUS_REJECTED:{
								tvOrdersheetHeader.setText("Movement Code: - "+loadRequestDO.MovementCode+" (Rejected)");
								if(load_type == AppConstants.LOAD_STOCK)
									showCustomDialog(PreviewNewLoadRequest.this, "Alert!", "Your Load request has been rejected by admin.", "OK", null, "finish");
								else
									showCustomDialog(PreviewNewLoadRequest.this, "Alert!", "Your Unload request has been rejected by admin.", "OK", null, "finish");
			...
			showCustomDialog(PreviewNewLoadRequest.this, "Warning !", getString(R.string.no_internet), "OK", null, "");
	}
...
//			productDO.etQty = viewHolder.etQt;s
	...
	private static class ViewHolder
	{
	 ...
	 {
		 CustomBuilder customBuilder = new CustomBuilder(PreviewNewLoadRequest.this, title, false);
